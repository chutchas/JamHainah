...
/**
 * กด "ยังก่อน" ตอนถูกชวนเพิ่มเอกสาร
 *
 * เขาปฏิเสธการเพิ่มใบใหม่ ไม่ได้ปฏิเสธเรา — ปุ่มที่เหลือจึงต้องไม่ใช่ปุ่มชวนเพิ่มอีก
 * แต่ต้องมีทางไปต่อสักทาง ไม่งั้นบทสนทนาจบลงที่ประโยคเดียวแล้วตัน
 *
 * เดิมข้อความนี้เขียนดิบ ๆ อยู่ใน handlers จึงไม่ผ่าน chips() ไม่มีปุ่ม
 * และมี emoji ของระบบหลุดรอดเทสต์มาได้ — ข้อความที่เขียนนอกไฟล์นี้
 * คือข้อความที่ไม่มีใครกลับมาดูแล
 */
/**
 * ส่งมาหลายใบ แต่บางใบอ่านไม่ออก
 *
 * ต้องบอกจำนวนให้ตรง เพราะเขานับรูปที่ส่งไปเองได้
 * เงียบเรื่องใบที่หาย = เขาคิดว่าระบบเก็บครบแล้ว แล้วไปรู้ตอนของหมดอายุ
 */
export function someUnreadable(names: string[]): LineMessage {
  return text(
    `อีก ${names.length} ใบผมยังบันทึกไม่ได้ครับ (${names.join(' · ')})\n` +
      'ส่งใบนั้นมาใหม่ทีละใบได้ไหมครับ'
  );
}

export function later(): LineMessage[] {
  return [text('ได้ครับ ส่งมาเมื่อไหร่ก็ได้', chips([{ label: LIST_NAME, liff: true, icon: 'doc' }]))];
}

/** กดยกเลิกกลางทาง — ต้องไม่ทิ้งเขาไว้กับหน้าจอที่ไม่มีอะไรให้กดต่อ */
